...
cleaned AS (
  SELECT
    ...
    CASE
      ...
      CASE
        WHEN cleaned_price < 50 THEN 'Budget'
        WHEN cleaned_price BETWEEN 50
        AND 150 THEN 'Mid-Range'
        WHEN cleaned_price BETWEEN 151
        AND 300 THEN 'Premium'
        WHEN cleaned_price > 300 THEN 'Luxury'
        ELSE 'Unknown'
      ...
      END AS stay_type,
      CASE
        WHEN cleaned_price > 0
        AND minimum_nights > 0 THEN TRUE
        ELSE FALSE
      END AS is_bookable,
        CAST(
          created_at AS TIMESTAMP
        ) AS listing_created_at,
        CAST(
          updated_at AS TIMESTAMP
        ) AS listing_updated_at,
        CURRENT_TIMESTAMP() AS dbt_loaded_at,
        'stg_listing' AS dbt_source_model
        FROM
          price_cleaned
        WHERE
          id IS NOT NULL
